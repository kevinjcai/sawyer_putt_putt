...
def ik_service_client():
    ...
    user_input = input("Enter desired x y z: ")
    user_input = [float(i) for i in user_input.split(' ')]
    pose_stamped.pose.position.x = user_input[0]
    pose_stamped.pose.position.y = user_input[1]
    pose_stamped.pose.position.z = user_input[2]
